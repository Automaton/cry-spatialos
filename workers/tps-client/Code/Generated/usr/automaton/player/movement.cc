...
bool MovementData::running() const {
  return _running;
}

bool& MovementData::running() {
  return _running;
}

MovementData& MovementData::set_running(bool value) {
  _running = value;
  return *this;
}
...
Movement::Update Movement::Update::FromInitialData(const ::automaton::player::MovementData& data) {
  ...
  update._running.emplace(data.running());
  return update;
}
...
const ::worker::Option< bool >& Movement::Update::running() const {
  return _running;
}

::worker::Option< bool >& Movement::Update::running() {
  return _running;
}

Movement::Update& Movement::Update::set_running(bool value) {
  _running.emplace(value); return *this;
}
...
void Write_MovementData(
  ...
  ::Pbio_AddBool(object, 3, static_cast<std::uint8_t>(data.running()));
}
...
::automaton::player::MovementData Read_MovementData(::Pbio_Object* object) {
  ...
  auto _running = ::Pbio_GetBool(object, 3) != 0;
  return {
      std::move(_moving),
  ...
  if (object_type == ::worker::detail::ClientComponentVtable::Update) {
    *object = new ::automaton::player::Movement::Update;
    auto client_object = static_cast< ::automaton::player::Movement::Update* >(*object);
    {
      ...
      if (::Pbio_GetBoolCount(state_object, 3)) {
        auto _running = ::Pbio_GetBool(state_object, 3) != 0;
        client_object->running().emplace(std::move(_running));
      }
    }
    {
    }
...
void VtableSerialize_Movement(
  ...
  if (object_type == ::worker::detail::ClientComponentVtable::Update) {
    const auto& data = *static_cast< const ::automaton::player::Movement::Update* >(object);
    {
      ...
      if (data.running()) {
        ::Pbio_AddBool(state_object, 3, static_cast<std::uint8_t>((*data.running())));
      }
    }
    {
    }
